            ...
            <div class="col-md-8">
                <div class="card border-light shadow-sm mb-3">
                    <div class="card-header" style="background-color: #000000; color: #FFD700;">
                        <h5 class="mb-0">{{ __('Actualizar Número de Caja') }}</h5>
                    </div>

                    <div class="card-body" style="background-color: #f0f0f0;">
                        <form action="{{ route('num-cajas.update', $numCaja->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="estado" id="estado" class="form-control">
                                    <option value="activa" {{ $numCaja->estado == 'activa' ? 'selected' : '' }}>{{ __('Activa') }}</option>
                                    <option value="inactiva" {{ $numCaja->estado == 'inactiva' ? 'selected' : '' }}>{{ __('Inactiva') }}</option>
                                </select>
                            </div>

                            <div class="card-footer text-center">
                                <button type="submit" class="btn btn-warning btn-lg">{{ __('Actualizar Caja') }}</button>
